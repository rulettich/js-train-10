...
function customAssign(...objects) {
  ...
  for (const key in result) {
    if (typeof result[key] === "number") {
      result[key] = result[key].toString().replace(/ /g, "_");
    }
  }
...
function customEntries(obj) {
  ...
  if (typeof obj !== "object" || obj === null) {
    throw new Error("Помилка: Аргумент не є об'єктом.");
  }
...
function customObjectFromEntries(entries) {
  ...
  if (!Array.isArray(entries)) {
    throw new Error("Помилка: Вхідний аргумент має бути масивом.");
  }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (Object.isFrozen(newObj)) {
    ...
    throw new Error("Помилка: Не вдалося заморозити об'єкт.");
  }
}
 ...
 *
 *  obj - Об'єкт, для якого потрібно обчи,якщо ні повертаємо 0
  // Отримуємо всі значення об'єкта
  // Обчислюємо суму значень
  // Повертаємо сумуслити суму значень.
 * Повертаємо - Сума всіх значень об'єкта.
 */
function getObjectValuesSum(obj) {
  // Перевірка, чи є аргумент об'єктом
  if (typeof obj !== "object" || typeof obj === null) {
    return 0;
  }

  const values = Object.values(obj);
  const sum = values.reduce((acc, currVal) => {
    return acc + currVal;
  }, 0);
